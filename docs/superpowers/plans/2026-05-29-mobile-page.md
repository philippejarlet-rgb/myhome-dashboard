# Mobile Page Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Créer `app/mobile/page.tsx` — une page smartphone avec 3 onglets (MyHome, Courses, Todo) utilisant les mêmes données Supabase que le dashboard.

**Architecture:** Un seul fichier client `app/mobile/page.tsx` avec état local `activeTab`. Chaque onglet charge ses données via les API existantes `/api/data/todos` et `/api/data/courses`. L'auth existante (middleware) protège la route automatiquement.
...
| Fichier | Action |
|---|---|
| `app/mobile/page.tsx` | Créer — page complète avec 3 onglets |

---

### Task 1: Créer la page mobile

**Files:**
- Create: `app/mobile/page.tsx`

- [ ] **Step 1: Créer `app/mobile/page.tsx`**
...
- [ ] **Step 3: Vérifier sur `http://localhost:3000/mobile`**

- Les 3 onglets s'affichent en haut
- Onglet MyHome : logo + "MYHOME"
- Onglet Courses : liste + input + bouton +
- Onglet Todo : liste + input + bouton +
- Cocher un item le barre
- Supprimer un item le retire de la liste
- Les données persistent après reload (Supabase)

- [ ] **Step 4: Commit**

```bash
git add app/mobile/page.tsx
git commit -m "feat: add /mobile page with Todo and Courses tabs"
```

---

### Task 2: Push et déploiement
...
- [ ] **Step 2: Tester sur iPhone**

Ouvrir `myhome.lpj.ch/mobile` dans Safari → "Partager" → "Sur l'écran d'accueil" → raccourci direct vers la page mobile.
